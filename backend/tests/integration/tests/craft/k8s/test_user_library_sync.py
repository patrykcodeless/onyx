"""User-library API sync tests in the full Craft k8s integration lane.
...
class TestUserLibrarySync:
    ...
    def test_delete_api_removes_file_from_running_sandbox(
        ...
        delete_response = _delete(handle.api_user, document_id)
        delete_response.raise_for_status()
        assert delete_response.json()["deleted"] == document_id
        assert all(entry["id"] != document_id for entry in _tree(handle.api_user))
